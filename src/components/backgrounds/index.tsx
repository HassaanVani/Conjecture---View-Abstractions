...
// Calculus: Flowing curves with tangent lines and integral area fills
    ...
    useEffect(() => {
        ...
        const animate = () => {
            ...
            for (const curve of curves) {
                ...
                // Integral shading under one curve
                if (curve === curves[0]) {
            ...
            // Floating calculus symbols
            ctx.font = '14px serif'
            ctx.fillStyle = 'rgba(180, 120, 255, 0.05)'
            const symbols = ['∫', 'dx', "f'", '∂', 'lim', '∞', 'Σ', 'δ', 'ε', 'dy']
            for (let i = 0; i < 10; i++) {
                const x = ((i * 127 + frame * 0.25) % (canvas.width + 50)) - 25
                const y = ((i * 83 + Math.sin(frame * 0.009 + i) * 40) % canvas.height)
                ctx.fillText(symbols[i % symbols.length], x, y)
            }
